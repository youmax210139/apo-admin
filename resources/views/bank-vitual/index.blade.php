...
@section('content')
    ...
    <div class="row">
        <div class="row page-title-row" id="dangqian" style="margin:5px;">
            ...
            <div class="col-md-6 text-right">
                <a href="/bankvirtual/create" class="btn btn-primary btn-md"><i class="fa fa-plus-circle"></i> 添加银行 </a>
            </div>
        </div>
        <div class="col-sm-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">虚拟货币银行管理</h3>
                </div>
                    ...
                    <form class="form-horizontal" id="defaultForm" role="form" method="POST">
                        ...
                        <table id="tags-table" class="table table-striped table-condensed table-bordered table-hover app_w100pct">
                            ...
                            <tr>
                                ...
                                <th data-sortable="false">币别</th>
                                <th data-sortable="false">汇率</th>
                                <th data-sortable="false">汇率API</th>
                                <th data-sortable="false">使用渠道</th>
                                <th data-sortable="false">允许提现</th>
                                <th data-sortable="false">禁用</th>
                                <th data-sortable="false">汇率模式</th>
                                <th data-sortable="false">操作</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse($bank_virtual as $v)
                                <tr>
                                    ...
                                    <span class="label label-info">
                                        @if($v->api_fetch==0)
                                            人工汇率
                                        @elseif($v->api_fetch==1)
                                            RBCX汇率
                                        @elseif($v->api_fetch==2)
                                            OTC365汇率
                                        @endif
                                    </span>
                                    </td>
                                    <td>
                                        <a href="/bankvirtual/edit?id={{$v->id}}" class="X-Small btn-xs text-primary">编辑</a>
                                        @if($v->disabled)
        ...
        <div class="modal-dialog modal-primary">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="lead">
                        <i class="fa fa-question-circle fa-lg"></i>
                        确认要<span class="row_verify_text"></span>该虚拟货币银行吗?
                    </p>
                </div>
                <div class="modal-footer">
    ...
    <script>
        $(function () {
            ...
            $("table").delegate('.del', 'click', function () {
                ...
                $('.deleteForm').attr('action', '/bankvirtual/?id=' + id);
                $("#modal-delete").modal();
            });
            $("table").delegate('.disabled', 'click', function () {
                ...
                $('.verifyForm').attr('action', '/bankvirtual/disabled/?id=' + id);
