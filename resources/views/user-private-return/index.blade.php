                ...
                <div class="box-body">
                    ...
                    <table id="tags-table"
                        ...
                        <tr>
                            ...
                            <th class="col-sm-1" data-sortable="false">私返权限</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="lead">
                        ...
                        确认要删除 <span class="row_verify_text">该会员</span> 的私返契约吗?
                    </p>
                </div>
            ...
            function showParentTree(data) {
                ...
                if(data) {
                    for(var i=0; i<data.length; i++) {
                        str += '<li><a href="/userprivatereturn/?username=' + data[i].username + '" class="X-Small btn-xs text-primary">'+ data[i].username +'</a></li>';
                    }
                }
                ...
                columnDefs: [
                    {
                        ...
                        'render': function (data, type, row) {
                            if (typeof row.self !== 'undefined' && row.self === 1) {
                                ...
                                if (row['user_observe']) {
                                    return '<a href="/userprivatereturn/?username=' + row.username + '" class="label-danger">' + row.username + '</a>';
                                } else {
                                    return '<a href="/userprivatereturn/?username=' + row.username + '" >' + row.username + '</a>';
                                }
                            }
                        }
                    },
                    ...
                    },
                    {
                        ...
                        'render': function (data, type, row) {
                            var str = '已开通';
                            var label = 'label-success';
                            if (row.user_private_return == "" || row.user_private_return == null) {
                                str = '未开通';
                                label = 'label-warning';
                            }

                            return app.getLabelHtml(str, label);
                        }
                    },
                    {
                        'targets': -1,
                        'render': function (data, type, row) {
                            var str = '';
                            str += '<a href="/userprivatereturn/edit/?user_id=' + row.user_id + '" class="btn-xs text-primary">设置契约</a>';
                            str += '<a href="/userprivatereturn/record/?username=' + row.username + '&user_id=' + row.user_id + '" class="btn-xs text-primary">契约调整记录</a>';
                            str += '<a href="javascript:;" user_id="' + row.user_id + '" username="' + row.username + '" class="btn-xs text-danger del">清除私返契约</a>';
                            return str;
                        }
                    }
                ]
