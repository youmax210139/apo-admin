...
class UserbetipController extends Controller
{
    public function getIndex()
    {
        $data['start_date'] = Carbon::today()->subDays(7);
        $data['end_date'] = Carbon::today()->endOfDay();

        return view('user.betip', $data);
    }
    ...
    public function postIndex(UserIpIndexRequest $request)
    {
        if ($request->ajax()) {
            ...
            $subQuery = Projects::select([DB::raw('max(id) as id'), 'user_id', DB::raw('sum(total_price) as total_price'), DB::raw('count(user_id) as total')])
            ...
            $data['recordsTotal'] = $data['recordsFiltered'] = DB::table(DB::raw("({$subQuery->toSql()}) as ull1"))
                ...
                ->leftJoin('projects as ull', 'ull.id', 'ull1.id')->count();

            $data['data'] = DB::table(DB::raw("({$subQuery->toSql()}) as ull1"))
                ->mergeBindings($subQuery->getQuery())
                ->leftJoin('projects as ull', 'ull.id', 'ull1.id')
                ...
                ->select(['ull.user_id', 'u.user_group_id', 'ull1.total', 'ull1.total_price', 'u.username', 'tu.username as top_username', 'ull.ip', 'uf.balance', 'ull.created_at'])
                ->get();
