...
class CreateTableThirdGameAviaBet extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('third_game_avia_bet', function (Blueprint $table) {
            $table->increments('id');
            $table->string('third_game_ident', 32)->default('')->comment('游戏接口ident');
            $table->integer('user_id')->comment('用户ID');
            $table->string('game', 256)->comment('游戏名称或标识');
            ...
            $table->timestamp('match_date_time')->nullable()->comment('比赛时间');
    ...
    public function down()
    {
        if (app()->environment() != 'production') {
            Schema::dropIfExists('third_game_avia_bet');
        }
    }
}
